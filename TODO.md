...
- [~] **W6.6 — GitHub App** (webhook receiver + outbound ack comment shipped; verdict-on-completion + Checks API deferred)
  ...
  - Pending: posting the verdict back as a comment when the run completes (needs a worker hook to call post_pr_comment after `process_pending_run` succeeds); GitHub Checks API integration; opening PRs as `claudeStruct[bot]`
