...
### Added — Wave 10 (GX10 / local-first)
- **`cs voice` — local Whisper REPL** (W10.7): `cs voice transcribe` records from the default mic and prints the transcription; `cs voice run <task>` records + transcribes + invokes `cs dev/review/plan/debug` with the captured text. Lazy imports for `sounddevice`/`numpy`/`faster-whisper` keep the base install lean; missing `[voice]` extra raises a clear `VoiceError` pointing at the install command. Module-level model cache so repeated calls reuse loaded weights. Defaults targeted at GX10's compute budget (`base.en`, 16 kHz mono, 5 s capture, auto device).
- **`--smart-context` — RAG file selection** (W10.5): new `cs index build/stats/clear` subcommands; new `--smart-context` flag on `cs dev/review/plan/debug` plumbs the description into a top-K (k=20) embedding-similarity lookup against `~/.claudestruct/index/<repo>.db` and feeds the result to the existing gatherer's `explicit_paths`. Pure-Python cosine; defends against zero-norm rows and dim-mismatch. `embed.py` is stdlib-only (urllib + json), targeting any OpenAI-compatible `/embeddings` endpoint (Ollama / vLLM / SGLang / cloud OpenAI).
- **Nightly code-health watchdog** (W10.8): `scripts/nightly-review.sh` walks the last N commits of a configured branch, runs `cs review` per commit (detach + soft-reset to isolate the single-commit diff), drops JSONL logs under `.claudestruct/runs/`. `deploy/systemd/claudestruct-nightly.{service,timer}` fires it nightly with `Persistent=true`.
- **Real `CLONE_NEWNET` isolation in claw-sandbox** (W10.9): `--no-network` actually unshares the netns now (Linux + CAP_SYS_ADMIN). Auto-on by default when the host can enforce; new `--allow-network` opts out for `npm install` workflows. `claw-squad/docs/sandbox-hardening.md` rewritten with a host-config matrix.
- **Always-on home-server deploy** (W10.2): `deploy/systemd/{claudestruct,claudestruct-worker,claw-squad}.service` + `docs/home-server.md` cover Tailscale Funnel and Caddy reverse-proxy + Let's Encrypt. Idempotent first-boot via `ExecStartPre=cs serve init-db`; standard systemd hardening.
- **`cs dataset export` + LoRA fine-tune scaffold** (W10.6): opt-in `run.io` event captures task / model / description / responseText (gated on `CLAUDESTRUCT_LOG_PROMPTS=1`, default off for privacy). New `cs dataset export --out PATH [--task review] [--since YYYY-MM-DD] [--format alpaca|chat]` mines `.claudestruct/runs/*.jsonl` for fine-tuning data. `scripts/finetune-reviewer.sh` is a thin axolotl wrapper targeting Qwen2.5-Coder-7B QLoRA on a GX10.
- **Hybrid cloud / local routing** (W10.10): `claw-squad/configs/hybrid.json` + `claw-squad/docs/hybrid-routing.md`. Planner on cloud Anthropic (asymmetric IQ demand + low call volume + prompt-cache savings), Coder + Reviewer on local Ollama. ~25 % of all-cloud spend on a typical multi-TODO run.
- **Local prompt-result cache** (W9.4 + W10.4): `src/claudestruct/local_cache.py` content-hashes `(provider, model, system, messages, effort, max_tokens)` to a SHA-256 keyed JSON file under `~/.claudestruct/llm_cache/<aa>/<sha>.json`. `client.run_task` short-circuits on hit and replays through `stream_callback`. `--llm-cache` / `--no-llm-cache` flags + `is_enabled_for(provider)` policy (auto: ON for openai-compat, OFF for Anthropic).
- **OpenAI-compat provider + per-role local presets + hybrid recipe** (W10.1 + W10.3 + W10.10): `claudestruct.providers` Protocol + `OpenAICompatProvider`; runtime selection via `CLAUDESTRUCT_PROVIDER=anthropic|openai`. `claw-squad/configs/{local-gx10,local-laptop,hybrid}.json` shipped alongside the binary; `--preset <name>` resolves them.
